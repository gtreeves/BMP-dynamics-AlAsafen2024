function [nucstats,xnuc,ynuc,snuc,w,mask,U,CC_size] = find_nuclei3_c2(I,Ip,lims,xp,yp,scalings,varargin)
...
[bgm fgm] = ridgelines(U); 
% [bgm fgm] = ridgelines3(U); 

% % Getting the stats about each region as separated by ridge lines
stats_bgm_com = regionprops(imcomplement(bgm),'Centroid','PixelList','PixelIdxList');
...
for  i = 1:num_comp
    ...
    temp_img = imbinarize(temp_img); % converting to binary image
    
    %Cleaning the binarized image
    temp_img = imopen(temp_img,strel('disk',2));
    temp_img = bwareaopen(temp_img, 400);
...
stats = regionprops(temp_final_img,U2,'Centroid','PixelList','PixelIdxList','MeanIntensity');


% eliminating nuclei below a certain intensity
nuc_ints = [];

for sz = 1:size(stats,1)
    
   nuc_ints(sz,1) = stats(sz).MeanIntensity; 
    
    
end

nuc_rng = prctile(nuc_ints,lims);

nuc_indcs = find(nuc_ints<nuc_rng(1));



temp_final_img2 = temp_final_img;

for i = 1: size(nuc_indcs)
    
   pxl_list = stats(nuc_indcs(i)).PixelList;
    
   
   for j = 1:size(pxl_list,1)
        
        temp_final_img2(pxl_list(j,2),pxl_list(j,1)) = 0;
...
stats = regionprops(temp_final_img2,'Centroid','PixelList','PixelIdxList');
...
% 	disp('job done')
	...
	if exist('filename','var') || exist('outfilename','var')
        ...
        if ~(exist('NucImages\OutlineMask','dir') == 7)
			mkdir NucImages\OutlineMask
        end

        if ~(exist('NucImages2\Nuclei,ridges, mask and maxima','dir') == 7)
			mkdir 'NucImages2\Nuclei,ridges, mask and maxima'
        end
...
% 	imshowcat(gcf,double(~~mask),mDU(double(I)),double(~~mask))
	...
	if exist('filename','var') && ~exist('outfilename','var')
		...
		print(gcf,['NucImages\OutlineMask',filesep,...
			filenameshort(1:end-4),'_',num2strDU(outidx,4),'.jpg'],'-djpeg','-r150')
		close(gcf)
...
% 		close(gcf)
        ...
        set(gca,'Position',[0 0 1 1])
		print(gcf,['NucImages\Nuclei,ridges, mask and maxima',filesep,...
			'U2mn_',filenameshort(1:end-4),'_',num2strDU(outidx,4),'.jpg'],'-djpeg','-r150')
		close(gcf)
